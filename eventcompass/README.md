...
- `POST /api/chats` — create a new chat (optionally with initial messages)
- `GET /api/chats/:id/messages` — get a single chat with messages
- `POST /api/chats/:id/messages` — append messages to a chat

The UI is in `src/components/CompassChat.tsx` and child components.

## Files added / important locations

- `prisma/schema.prisma` — Prisma schema defining `Chat` and `Message` models (SQLite datasource). Run `prisma generate` and `prisma migrate` after edits.
- `lib/prisma.ts` — Lightweight Prisma client wrapper to avoid multiple client instances in development.
- `src/app/api/chats/route.ts` — API handler (GET/POST) for listing and creating chats using Prisma.
- `src/app/api/chats/[id]/messages/route.ts` — API handler (GET/POST) for retrieving a chat and appending messages using Prisma.
