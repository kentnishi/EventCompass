...
export async function GET(req: Request) {
    ...
    try {
        ...
        const usable = (rows ?? []).filter(r =>
            [r.timing, r.structure, r.incentives, r.budgeting, r.score]
        ...
        const X = usable.map(r => [1, r.timing, r.structure, r.incentives, r.budgeting]);
        ...
        const x = [1, timing, structure, incentives, budgeting];
        ...
        return NextResponse.json({
            event_id: id,
            features: { timing, structure, incentives, budgeting, registration, location },
