...
export async function GET() {
    const { data, error } = await supabase
        .from('event_stats')
        .select('timing,structure,incentives,audience,location,registration,budgeting,score')
        .not('score', 'is', null);
    ...
    const rows = (data ?? []).filter(r =>
        [r.timing, r.structure, r.incentives, r.audience, r.location, r.registration, r.budgeting, r.score]
