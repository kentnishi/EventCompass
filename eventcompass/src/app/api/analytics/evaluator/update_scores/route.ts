...
function scoreStructure(eventType?: string | null, registered?: number | null) {
    ...
    const sizeFit = clamp01(1 - Math.abs(reg - ideal) / Math.max(ideal, 1));
...
function scoreAudience() { // placeholder until fields exist
    const demographic = 0, major = 0, gender = 0;
    return (demographic + major + gender) / 3;
}
